...
func TestDeveloperMsgServerDelete(t *testing.T) {
	creator := "A"

	for _, tc := range []struct {
		desc    string
		request *types.MsgDeleteDeveloper
		err     error
	}{
		{
			desc: "Completed",
			request: &types.MsgDeleteDeveloper{Creator: creator,
				Address: strconv.Itoa(0),
			},
		},
		{
			desc: "Unauthorized",
			request: &types.MsgDeleteDeveloper{Creator: "B",
				Address: strconv.Itoa(0),
			},
			err: sdkerrors.ErrUnauthorized,
		},
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			_, err := srv.CreateDeveloper(wctx, &types.MsgCreateDeveloper{Creator: creator,
				Address: strconv.Itoa(0),
			})
			require.NoError(t, err)
			_, err = srv.DeleteDeveloper(wctx, tc.request)
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
				_, found := k.GetDeveloper(ctx,
					tc.request.Address,
				)
